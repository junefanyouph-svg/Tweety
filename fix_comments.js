...
const regexMapStart = /\{comments\.filter\(c => !c\.parent_id\)\.map\(comment => \{/;
const startMatch = code.match(regexMapStart);
if (!startMatch) {
  console.log("Could not find map start.");
  process.exit(1);
}

const regexMapEnd = /          \}\)\}(?:\r?\n)+\s*<div className="mt-8 relative" ref=\{commentBoxRef\}>/;
const match = code.match(regexMapEnd);
if (!match) {
  console.log("Could not find map end with regex.");
  process.exit(1);
}

const newCodeSection = '{comments.filter(c => !c.parent_id).map(comment => renderCommentThread(comment))}';

code = code.substring(0, startMatch.index) + newCodeSection + code.substring(match.index + 13); // +13 to skip `          })}` but keep the rest

const renderFn = `const renderCommentThread = (comment, depth = 0, isLast = false) => {
              ...
              <div className="pill-input-outer relative">
                ...
                <div className="pill-input-container">
                  ...
                  <button
                    ...
                    onClick={() => handleComment(e, comment.id)}
                  >
...
if (!code.includes('const renderCommentThread =')) {
  ...
  if (!returnRegex.test(code)) {
    console.log("Could not find return statement. Exiting.");
    process.exit(1);
  }
  code = code.replace(returnRegex, '  ' + renderFn);

  fs.writeFileSync('client/src/components/PostCard.jsx', code);
  console.log("Success");
} else {
  console.log("renderCommentThread already exists, logic not changing");
}
